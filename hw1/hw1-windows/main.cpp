...
//	gluPerspective(90,w/(float)h,0.1,99);
	...
	mat4 x = glm::perspective((float)90, w / (float)h, (float)0.1, (float)99.0);
	...
	printf("\n");
	x = Transform::perspective(90, w / (float)h, 0.1, 99);
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
	...
	mat4 ret = Transform::perspective(90, w / (float)h, 0.1, 99); 
	glLoadMatrixf((float*)&ret[0]);
	glViewport(0,0,w,h);
}
...
void display() {
	...
	mat4 rotate = Transform::_rotate(30, vec3(1, 0, 0));
...
int main(int argc,char* argv[]) {
	...
	printHelp();
	glutMainLoop();	// 让所有与‘事件’有关的函数调用无限循环
